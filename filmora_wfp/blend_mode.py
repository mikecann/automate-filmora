"""Guarded replacement of an existing static overlay Blend Mode."""

from __future__ import annotations

import copy
import hashlib
import json
import os
import tempfile
import zipfile
from pathlib import Path
from typing import Any, Dict, MutableMapping, Optional, Union

from .archive import TIMELINE_SUFFIX, WfpError
from .diffing import diff_projects
from .evals import evaluate_project
from .scale import _clips
from .title_cards import _compact_json, _load_decimal_json

Pathish = Union[os.PathLike[str], str]
_OBSERVED_MODES = {"Normal", "Multiply", "Screen"}


def _sha256(path: Path) -> str:
    digest = hashlib.sha256()
    with path.open("rb") as source:
        for chunk in iter(lambda: source.read(1024 * 1024), b""):
            digest.update(chunk)
    return digest.hexdigest()


def _mode(value: Any, label: str) -> str:
    if not isinstance(value, str) or value not in _OBSERVED_MODES:
        raise WfpError(label + " must be one of the observed static Blend Mode names")
    return value


def _pip(clip: MutableMapping[str, Any]) -> Dict[str, Any]:
    raw = clip.get("pipBuf")
    if not isinstance(raw, str):
        raise WfpError("Selected clip does not expose a serialized pipBuf")
    try:
        value = json.loads(raw.rstrip("\x00"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise WfpError("Selected clip pipBuf is not parseable JSON") from exc
    if not isinstance(value, dict) or not isinstance(value.get("BlendMode"), str):
        raise WfpError("Selected clip does not expose a static string Blend Mode")
    if "BlendModeKeyFrame" in value:
        raise WfpError("Only static pipBuf Blend Mode is supported")
    return value


def _replace(clip: MutableMapping[str, Any], uid: str, old: str, new: str) -> bool:
    if clip.get("thisUId") != uid:
        return False
    if clip.get("type") != 1:
        raise WfpError("Selected Blend Mode target is not a type-1 video clip")
    pip = _pip(clip)
    current = _mode(pip["BlendMode"], "current Blend Mode")
    if current != old:
        raise WfpError("Selected clip Blend Mode does not match the expected value")
    pip["BlendMode"] = new
    encoded = json.dumps(pip, separators=(",", ":"), ensure_ascii=False)
    clip["pipBuf"] = encoded
    clip["pipBufSize"] = len(encoded.encode("utf-8")) + 1
    return True


def preflight_clip_blend_mode(source: Pathish, *, clip_uid: str, old_mode: Any,
                              new_mode: Any) -> Dict[str, Any]:
    source_path = Path(source).expanduser().resolve()
    if source_path.suffix.lower() != ".wfp" or not source_path.is_file():
        raise WfpError("Blend Mode replacement requires an existing .wfp source")
    if not isinstance(clip_uid, str) or not clip_uid:
        raise WfpError("clip_uid must be non-empty text")
    old, new = _mode(old_mode, "old_mode"), _mode(new_mode, "new_mode")
    if old == new:
        raise WfpError("new_mode must differ from old_mode")
    matches = 0
    with zipfile.ZipFile(source_path) as archive:
        for info in archive.infolist():
            if info.filename.endswith(TIMELINE_SUFFIX):
                document = _load_decimal_json(archive.read(info))
                for clip in _clips(document):
                    if _replace(copy.deepcopy(clip), clip_uid, old, new):
                        matches += 1
    if matches != 1:
        raise WfpError("Blend Mode selector must match exactly one existing overlay")
    return {"matching_archive_occurrences": matches, "old_mode": old, "new_mode": new}


def replace_clip_blend_mode(source: Pathish, output: Pathish, *, clip_uid: str,
                            old_mode: Any, new_mode: Any,
                            expected_source_sha256: Optional[str] = None) -> Dict[str, Any]:
    """Write a copy changing only an existing static Blend Mode string."""
    source_path, output_path = Path(source).expanduser().resolve(), Path(output).expanduser().resolve()
    if source_path == output_path or output_path.exists():
        raise WfpError("Refusing to overwrite the source or an existing output")
    preflight = preflight_clip_blend_mode(source_path, clip_uid=clip_uid,
                                          old_mode=old_mode, new_mode=new_mode)
    old, new = preflight["old_mode"], preflight["new_mode"]
    if expected_source_sha256 and _sha256(source_path).lower() != expected_source_sha256.lower():
        raise WfpError("Source fingerprint changed")
    output_path.parent.mkdir(parents=True, exist_ok=True)
    temporary = tempfile.NamedTemporaryFile(prefix=output_path.name + ".", suffix=".tmp",
                                             dir=output_path.parent, delete=False)
    temporary_path = Path(temporary.name)
    temporary.close()
    try:
        with zipfile.ZipFile(source_path) as source_zip, zipfile.ZipFile(temporary_path, "w") as destination:
            infos = source_zip.infolist()
            if len({info.filename for info in infos}) != len(infos):
                raise WfpError("Refusing to mutate an archive with duplicate member names")
            matches = 0
            for info in infos:
                data = source_zip.read(info)
                if info.filename.endswith(TIMELINE_SUFFIX):
                    document = _load_decimal_json(data)
                    changed = sum(_replace(clip, clip_uid, old, new) for clip in _clips(document))
                    if changed:
                        data = _compact_json(document).encode("utf-8")
                        matches += changed
                destination.writestr(info, data)
        if matches != 1:
            raise WfpError("Blend Mode selector did not match exactly one existing overlay")
        os.replace(temporary_path, output_path)
    finally:
        if temporary_path.exists():
            temporary_path.unlink()
    if not evaluate_project(output_path).get("valid"):
        output_path.unlink(missing_ok=True)
        raise WfpError("Generated Blend Mode copy failed format evaluation")
    audit = audit_clip_blend_mode_copy(source_path, output_path, clip_uid=clip_uid,
                                       old_mode=old, new_mode=new)
    if not audit["valid"]:
        output_path.unlink(missing_ok=True)
        raise WfpError("Generated Blend Mode copy failed semantic audit")
    return {"output": str(output_path), "format_eval_valid": True, "audit": audit,
            "matching_archive_occurrences": matches}


def audit_clip_blend_mode_copy(source: Pathish, output: Pathish, *, clip_uid: str,
                               old_mode: Any, new_mode: Any) -> Dict[str, Any]:
    try:
        result = diff_projects(Path(source), Path(output), max_changes=10000)
        old, new = _mode(old_mode, "old_mode"), _mode(new_mode, "new_mode")
    except WfpError as exc:
        return {"valid": False, "errors": [str(exc)]}
    errors = []
    if result.get("added_members") or result.get("removed_members"):
        errors.append("archive membership changed")
    changed = result.get("changed_members") or []
    if not changed or any(not item.endswith(TIMELINE_SUFFIX) for item in changed):
        errors.append("unexpected archive membership change")
    mode_changes = 0
    for change in result.get("json_changes") or []:
        path = str(change.get("path"))
        if path.endswith(".pipBuf.$embedded_json.BlendMode"):
            if change.get("before") != old or change.get("after") != new:
                errors.append("Blend Mode diff does not match requested values")
            mode_changes += 1
        elif path.endswith(".pipBufSize") or path.startswith("$.timelineInfos[0].userData[") or path.startswith("$.media_items.") or path.startswith("$.proj_") or path.startswith("$.project_"):
            continue
        else:
            errors.append("unexpected semantic change: " + path)
    if mode_changes != 1:
        errors.append("expected exactly one Blend Mode change")
    if not evaluate_project(Path(output)).get("valid"):
        errors.append("output failed format evaluation")
    return {"valid": not errors, "errors": errors,
            "details": {"mode_changes": mode_changes, "old_mode": old, "new_mode": new}}
