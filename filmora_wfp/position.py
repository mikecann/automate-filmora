"""Narrow replacement of existing Filmora video-clip position values."""
...
def _float32_decimal(value: Decimal) -> Decimal:
    """Match the 32-bit float Filmora writes for transform positions."""

    packed = struct.pack("f", float(value))
    return Decimal(str(struct.unpack("f", packed)[0]))


def normalized_position(x_pixels: DecimalInput, y_pixels: DecimalInput, width: int, height: int) -> Tuple[Decimal, Decimal]:
    """Convert visible Filmora pixel offsets to its normalized transform values."""
...
def _position_params(clip: MutableMapping[str, Any]) -> Dict[str, List[MutableMapping[str, Any]]]:
    params: Dict[str, List[MutableMapping[str, Any]]] = {"Position_x": [], "Position_y": []}
...
def _replace_position(
    ...
    if clip.get("thisUId") != clip_uid:
        return False
    if clip.get("type") != 1:
        raise WfpError("Selected position target is not a type-1 video clip")
    params = _position_params(clip)
    if len(params["Position_x"]) != 1 or len(params["Position_y"]) != 1:
        raise WfpError("Selected clip does not expose exactly one existing Position_x and Position_y parameter")
    current_x = _finite_decimal(params["Position_x"][0]["fxParam"]["unValue"], "current Position_x")
    current_y = _finite_decimal(params["Position_y"][0]["fxParam"]["unValue"], "current Position_y")
    if current_x != old_x or current_y != old_y:
        raise WfpError(
            "Selected clip position does not match: expected ({0}, {1}), found ({2}, {3})".format(
                old_x, old_y, current_x, current_y
            )
        )
    params["Position_x"][0]["fxParam"]["unValue"] = new_x
    params["Position_y"][0]["fxParam"]["unValue"] = new_y
    return True


def preflight_clip_position(
    source: Pathish,
    *,
    clip_uid: str,
    old_position_x: DecimalInput,
    old_position_y: DecimalInput,
    new_x_pixels: DecimalInput,
    new_y_pixels: DecimalInput,
) -> Dict[str, Any]:
    """Resolve one existing position target without writing."""

    source_path = Path(source).expanduser().resolve()
    if source_path.suffix.lower() != ".wfp" or not source_path.is_file():
        raise WfpError("Position replacement requires an existing .wfp source")
    if not isinstance(clip_uid, str) or not clip_uid:
        raise WfpError("clip_uid must be non-empty text")
    old_x = _finite_decimal(old_position_x, "old_position_x")
    old_y = _finite_decimal(old_position_y, "old_position_y")
    ...
    new_x, new_y = normalized_position(x_pixels, y_pixels, width, height)
    if old_x == new_x and old_y == new_y:
        raise WfpError("New position must differ from the current position")
    matches = 0
    with zipfile.ZipFile(source_path, "r") as archive:
        for info in archive.infolist():
            ...
            for clip in _clips(document):
                ...
                if _replace_position(candidate, clip_uid, old_x, old_y, new_x, new_y):
                    matches += 1
    if matches < 1:
        raise WfpError("Position selector did not match the source project")
    return {
        "matching_archive_occurrences": matches,
        "old_position_x": str(old_x),
        "old_position_y": str(old_y),
        "new_position_x": str(new_x),
        "new_position_y": str(new_y),
...
def _positions_for_uid(path: Path, clip_uid: str) -> List[Tuple[Decimal, Decimal]]:
        ...
        for info in archive.infolist():
            ...
            for clip in _clips(document):
                if clip.get("thisUId") != clip_uid:
                    continue
                params = _position_params(clip)
                if len(params["Position_x"]) == 1 and len(params["Position_y"]) == 1:
                    value = (
                        _finite_decimal(params["Position_x"][0]["fxParam"]["unValue"], "Position_x"),
                        _finite_decimal(params["Position_y"][0]["fxParam"]["unValue"], "Position_y"),
                    )
...
def audit_clip_position_copy(
    ...
    *,
    ...
    old_position_x: DecimalInput,
    old_position_y: DecimalInput,
    new_position_x: DecimalInput,
    new_position_y: DecimalInput,
) -> Dict[str, Any]:
    """Confirm that a generated copy changed only the selected position values."""

    source_path = Path(source).expanduser().resolve()
    output_path = Path(output).expanduser().resolve()
    old_x = _finite_decimal(old_position_x, "old_position_x")
    old_y = _finite_decimal(old_position_y, "old_position_y")
    new_x = _finite_decimal(new_position_x, "new_position_x")
    new_y = _finite_decimal(new_position_y, "new_position_y")
    errors: List[str] = []
    try:
    ...
    if result.get("added_members") or result.get("removed_members"):
        errors.append("Position copy changed archive membership")
    changed_members = result.get("changed_members") or []
    if not changed_members:
        errors.append("Position copy changed no archive members")
    if any(not member.endswith(TIMELINE_SUFFIX) for member in changed_members):
        errors.append("Position copy changed a non-timeline archive member")
    if result.get("parse_errors") or result.get("truncated"):
        errors.append("Position copy diff was incomplete")
    expected_pairs = set()
    if old_x != new_x:
    ...
    if observed_pairs != expected_pairs:
        errors.append("Position copy did not expose exactly the expected semantic changes")
    output_positions = _positions_for_uid(output_path, clip_uid)
    if output_positions != [(new_x, new_y)]:
        errors.append("Generated copy does not expose exactly one updated position")
    evaluation = evaluate_project(output_path)
    if not evaluation.get("valid"):
        errors.append("Generated position copy failed format evaluation")
    return {
        "valid": not errors,
        "errors": errors,
        "details": {
            "changed_members": changed_members,
            "position_values_changed": len(observed_pairs),
            "format_eval_valid": bool(evaluation.get("valid")),
        },
    }


def replace_clip_position(
    source: Pathish,
    output: Pathish,
    *,
    clip_uid: str,
    ...
    """Change existing video transform positions in a new WFP copy."""
    ...
    if source_path.suffix.lower() != ".wfp" or output_path.suffix.lower() != ".wfp":
        raise WfpError("Position replacement requires .wfp input and output paths")
    ...
    preflight = preflight_clip_position(
        source_path,
        clip_uid=clip_uid,
    ...
    old_x = Decimal(preflight["old_position_x"])
    old_y = Decimal(preflight["old_position_y"])
    new_x = Decimal(preflight["new_position_x"])
    new_y = Decimal(preflight["new_position_y"])
    ...
    try:
            ...
            with zipfile.ZipFile(temporary_path, "w") as destination:
                for info in infos:
                    ...
                    if info.filename.endswith(TIMELINE_SUFFIX):
                        ...
                        member_matches = 0
                        for clip in _clips(document):
                            if _replace_position(clip, clip_uid, old_x, old_y, new_x, new_y):
                                member_matches += 1
                        if member_matches:
        ...
        if not audit.get("valid"):
            ...
            raise WfpError(
                "Generated position copy failed source-aware audit: {0}".format(
