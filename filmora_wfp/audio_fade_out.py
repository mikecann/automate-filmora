...
def _positive_fade_time(value: FadeTimeInput, label: str) -> Decimal:
    ...
    if not result.is_finite() or result <= 0:
        raise WfpError("{0} must be a positive finite decimal number".format(label))
    # Filmora persists fade times as real-valued JSON numbers (for example 2.0).
    # Preserve that lexical shape even when an API caller supplies the integer 2.
    if result.as_tuple().exponent >= 0:
        result = result.quantize(Decimal("0.0"))
    return result
