"""Guarded replacement of an existing Filmora Video Denoise sigma value."""
...
def _replace(clip: MutableMapping[str, Any], uid: str, old: float, new: float) -> bool:
    if clip.get("thisUId") != uid:
        return False
    if clip.get("type") != 1:
        raise WfpError("Selected Video Denoise target is not a type-1 video clip")
    state = clip.get("image_denoise")
    if not isinstance(state, dict):
        raise WfpError("Selected clip does not expose an existing Video Denoise object")
    if set(state) != {"cache_mode", "sigma", "status", "version"}:
        raise WfpError("Selected Video Denoise object has an unfamiliar shape")
    if state.get("status") != 1 or state.get("version") != 1.0 or state.get("cache_mode") != 0:
        raise WfpError("Selected Video Denoise object is not an enabled normalized state")
    current = state.get("sigma")
    if isinstance(current, bool) or not isinstance(current, (int, float, Decimal)) or float(current) != old:
        raise WfpError("Selected Video Denoise sigma does not match the expected value")
    if isinstance(new, bool) or not isinstance(new, (int, float, Decimal)) or not 0 <= new <= 100:
        raise WfpError("Video Denoise sigma must be between 0 and 100")
    if new == old:
        raise WfpError("New Video Denoise sigma must differ from the current value")
    state["sigma"] = float(new)
    return True


def preflight_clip_video_denoise(source: Pathish, *, clip_uid: str, old_sigma: float,
                                 new_sigma: float) -> Dict[str, Any]:
    source_path = Path(source).expanduser().resolve()
    if source_path.suffix.lower() != ".wfp" or not source_path.is_file():
        raise WfpError("Video Denoise replacement requires an existing .wfp source")
        ...
        for info in archive.infolist():
            if info.filename.endswith(TIMELINE_SUFFIX):
                ...
                for clip in _clips(document):
                    if _replace(copy.deepcopy(clip), clip_uid, old_sigma, new_sigma):
                        matches += 1
    if matches != 1:
        raise WfpError("Video Denoise selector must match exactly one archive occurrence")
    return {"matching_archive_occurrences": matches, "old_sigma": old_sigma,
            "new_sigma": new_sigma}


def replace_clip_video_denoise(source: Pathish, output: Pathish, *, clip_uid: str,
                               old_sigma: float, new_sigma: float,
                               expected_source_sha256: Optional[str] = None) -> Dict[str, Any]:
    """Write a copy changing one existing enabled Video Denoise sigma value."""
    ...
    preflight_clip_video_denoise(source_path, clip_uid=clip_uid, old_sigma=old_sigma,
                                 new_sigma=new_sigma)
        ...
        with zipfile.ZipFile(source_path, "r") as source_zip, zipfile.ZipFile(
            ...
            for info in infos:
                ...
                if info.filename.endswith(TIMELINE_SUFFIX):
                    ...
                    for clip in _clips(document):
                        if _replace(clip, clip_uid, old_sigma, new_sigma):
                            matches += 1
                    if matches:
                        data = _compact_json(document).encode("utf-8")
                destination.writestr(info, data)
        if matches != 1:
            raise WfpError("Video Denoise selector did not match exactly one archive occurrence")
    ...
    if not evaluate_project(output_path).get("valid"):
        ...
        raise WfpError("Generated Video Denoise copy failed format evaluation")
    audit = audit_clip_video_denoise_copy(source_path, output_path, clip_uid=clip_uid,
                                          old_sigma=old_sigma, new_sigma=new_sigma)
    if not audit["valid"]:
        output_path.unlink(missing_ok=True)
        raise WfpError("Generated Video Denoise copy failed semantic audit")
    return {"output": str(output_path), "format_eval_valid": True, "audit": audit,
            "matching_archive_occurrences": matches}


def audit_clip_video_denoise_copy(source: Pathish, output: Pathish, *, clip_uid: str,
                                  old_sigma: float, new_sigma: float) -> Dict[str, Any]:
    try:
    ...
    for change in result.get("json_changes", []):
        ...
        if path.endswith(".image_denoise.sigma"):
            continue
    ...
    return {"valid": not errors, "errors": errors,
            "details": {"old_sigma": old_sigma, "new_sigma": new_sigma,
